...
#佈署
    ...
    #如果存在加載_雷達站，就強加載該區塊
    execute if entity @e[tag=anti_aircraft_missile_radar_forceload] at @e[tag=anti_aircraft_missile_radar_forceload] run forceload add ~ ~ ~ ~

#例外狀況

    #如果不存在雷達站，就刪除該區塊的強加載
    execute unless entity @e[tag=anti_aircraft_missile_radar] at @e[tag=anti_aircraft_missile_radar_forceload] run forceload remove ~ ~ ~ ~
